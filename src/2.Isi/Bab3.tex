...
\centering
\begin{tikzpicture}[
    hub/.style={rectangle, draw, rounded corners, minimum width=3.6cm, minimum height=0.9cm, align=center},
    box/.style={rectangle, draw, align=left, font=\footnotesize, text width=3.3cm, inner sep=6pt, rounded corners},
    arr/.style={-Stealth}
]

\node[hub] (login) {Login};
\node[hub, below=0.6cm of login] (menu) {Main menu (dashboard)};
\draw[arr] (login) -- (menu);
\draw[arr] (menu) -- ++(0,-0.8);

\node[box, below=1.4cm of menu, xshift=-5.7cm] (m1) {\textbf{Registration}\\{\scriptsize\textit{employees.*}}\\[2pt]
Daftar pegawai\\Tambah pegawai\\Edit pegawai\\Cetak ID card\\Restore data};

\node[box, right=0.35cm of m1] (m2) {\textbf{Permit letter}\\{\scriptsize\textit{permits.*}}\\[2pt]
Scan dan cari pegawai\\Simpan surat izin\\Cetak surat jalan\\Riwayat surat izin};

\node[box, right=0.35cm of m2] (m3) {\textbf{Transport claim}\\{\scriptsize\textit{claims.* (gate)}}\\[2pt]
Input dan cek kelayakan\\Simpan klaim\\Cetak klaim (batch)\\Riwayat klaim};

\node[box, right=0.35cm of m3] (m4) {\textbf{Manage rates}\\{\scriptsize\textit{rates.*}}\\[2pt]
Daftar dan tambah rute\\Edit tarif\\Arsipkan tarif lama\\Lihat dokumen};

\node[box, below=0.4cm of m1] (m5) {\textbf{Approval pegawai}\\{\scriptsize\textit{approvals.* (admin)}}\\[2pt]
Daftar pegawai pending\\Proses approval massal};

\node[box, below=0.4cm of m2] (m6) {\textbf{User management}\\{\scriptsize\textit{users.* (admin)}}\\[2pt]
Daftar pengguna\\Tambah pengguna\\Atur hak akses (RBAC)};

\node[box, below=0.4cm of m3] (m7) {\textbf{Dashboard analytics}\\{\scriptsize\textit{dashboard.v2}}\\[2pt]
Ringkasan visual TK\\Sinkronisasi lokasi manual};

\node[box, below=0.4cm of m4] (m8) {\textbf{Profile}\\{\scriptsize\textit{profile.*}}\\[2pt]
Edit profil akun\\Hapus akun};

\draw[dashed, rounded corners]
  ($(m1.north west)+(-0.3,0.5)$) rectangle ($(m8.south east)+(0.3,-0.3)$);
\node[font=\footnotesize, anchor=north west] at ($(m1.north west)+(-0.25,0.45)$) {Modul dalam Main Menu};
...
\begin{tikzpicture}[
    ...
    terminator/.style={rounded rectangle, draw, minimum width=2.2cm, minimum height=0.9cm, align=center},
    io/.style={trapezium, trapezium left angle=70, trapezium right angle=110, draw, minimum width=3.2cm, minimum height=0.9cm, align=center},
    process/.style={rectangle, draw, minimum width=4.2cm, minimum height=1.1cm, align=center},
    decision/.style={diamond, draw, aspect=2, minimum width=3.4cm, align=center, inner sep=1pt},
    reject/.style={rectangle, draw, minimum width=3.2cm, minimum height=1.1cm, align=center},
    arr/.style={-Stealth}
]
\node[terminator] (start) {Mulai};
\node[io, below=of start] (input) {Ajukan klaim IN};
\node[decision, below=of input] (d1) {Data lengkap?};
\node[decision, below=of d1] (d2) {Kerja $\geq$ 60 hari?};
\node[decision, below=of d2] (d3) {Sudah klaim IN?};
\node[decision, below=of d3] (d4) {Klaim IN $<$1 tahun?};
\node[process, below=of d4] (simpan) {Simpan klaim\\\footnotesize Hitung nominal otomatis};
\node[terminator, below=of simpan] (selesai1) {Selesai};

\node[reject, right=3.4cm of d3] (tolak) {Tolak klaim\\\footnotesize Tampilkan alasan penolakan};
\node[terminator, below=of tolak] (selesai2) {Selesai};

\draw[arr] (start) -- (input);
\draw[arr] (input) -- (d1);
\draw[arr] (d1) -- node[right, font=\footnotesize]{Ya} (d2);
\draw[arr] (d1.east) -| node[above, pos=0.25, font=\footnotesize]{Tidak} (tolak.north -| d1.east) -- (tolak.north);
\draw[arr] (d2) -- node[right, font=\footnotesize]{Ya} (d3);
\draw[arr] (d2.east) -| (tolak.north -| d2.east);
\draw[arr] (d3) -- node[right, font=\footnotesize]{Ya} (d4);
\draw[arr] (d3.west) -| node[above, pos=0.5, font=\footnotesize]{Tidak, pertama kali} ++(-2.4,0) |- (simpan.west);
\draw[arr] (d4) -- node[right, font=\footnotesize]{Tidak} (simpan);
\draw[arr] (d4.east) -| node[above, pos=0.5, font=\footnotesize]{Ya, sudah 1x} (tolak.south);
\draw[arr] (simpan) -- (selesai1);
\draw[arr] (tolak) -- (selesai2);
\end{tikzpicture}
...
\node[terminator] (start) {Mulai};
\node[io, below=of start] (input) {Ajukan klaim OUT};
\node[decision, below=of input] (d1) {Dept. Plantation?};
\node[decision, below=of d1] (d2) {Data lengkap?};
\node[decision, below=of d2] (d3) {Kerja $\geq$ 90 hari?};
\node[decision, below=of d3] (d4) {Sudah CLAIM\_IN?};
\node[decision, below=of d4] (d5) {Sudah CLAIM\_OUT?};
\node[process, below=of d5] (simpan) {Simpan klaim OUT\\\footnotesize Hitung nominal otomatis};
\node[terminator, below=of simpan] (selesai1) {Selesai};

\node[reject, right=3.4cm of d3] (tolak) {Tolak klaim OUT\\\footnotesize Tampilkan alasan penolakan};
\node[terminator, below=of tolak] (selesai2) {Selesai};

\draw[arr] (start) -- (input);
\draw[arr] (input) -- (d1);
\draw[arr] (d1) -- node[right, font=\footnotesize]{Tidak} (d2);
\draw[arr] (d1.east) -| node[above, pos=0.25, font=\footnotesize]{Ya} (tolak.north -| d1.east) -- (tolak.north);
\draw[arr] (d2) -- node[right, font=\footnotesize]{Ya} (d3);
\draw[arr] (d2.east) -| node[above, pos=0.25, font=\footnotesize]{Tidak} (tolak.north -| d2.east);
\draw[arr] (d3) -- node[right, font=\footnotesize]{Ya} (d4);
\draw[arr] (d3.east) -| (tolak.west);
\draw[arr] (d4) -- node[right, font=\footnotesize]{Ya} (d5);
\draw[arr] (d4.east) -| node[above, pos=0.25, font=\footnotesize]{Tidak} (tolak.south -| d4.east);
\draw[arr] (d5) -- node[right, font=\footnotesize]{Tidak} (simpan);
\draw[arr] (d5.east) -| node[above, pos=0.5, font=\footnotesize]{Ya} (tolak.south);
\draw[arr] (simpan) -- (selesai1);
\draw[arr] (tolak) -- (selesai2);
\end{tikzpicture}
...
Kode \ref{lst:approval} menunjukkan method \texttt{isComplete()} yang memeriksa 17 \textit{field} wajib secara \textit{looping}, dan langsung mengembalikan nilai \texttt{false} pada iterasi pertama begitu ditemukan \textit{field} yang kosong. Method ini dipanggil di dua titik berbeda: pada antarmuka untuk menonaktifkan \textit{checkbox} persetujuan, dan pada proses \textit{backend} saat \textit{approval} massal dieksekusi. Validasi ganda pada sisi \textit{backend} ini penting sebagai lapisan keamanan tambahan, karena mencegah karyawan dengan data tidak lengkap berstatus \textit{Aktif} meskipun ada upaya manipulasi permintaan (\textit{request}) dari sisi klien.
...
Kode \ref{lst:dedup} menunjukkan validasi tiga lapis yang dijalankan terhadap setiap baris data sebelum diunggah ke basis data. Lapis pertama memastikan \texttt{NIK} berupa 16 digit angka murni menggunakan ekspresi reguler (\textit{regex}). Lapis kedua memeriksa duplikasi di dalam berkas yang sama menggunakan \texttt{in\_array}, sedangkan lapis ketiga memeriksa duplikasi terhadap data yang sudah tersimpan di basis data melalui \texttt{Employee::where('nik', \$nik)->exists()}. Rekaman yang lolos ketiga pemeriksaan dikumpulkan pada \texttt{\$validRows}, sementara rekaman bermasalah dicatat secara terpisah pada \texttt{\$problemRows} untuk dilaporkan kembali kepada klien, bukan dihapus begitu saja tanpa jejak. Pendekatan ini memastikan proses migrasi tetap dapat ditelusuri dan tidak kehilangan data secara diam-diam.